 ...
 * 
 */
package com.onlinebooking.bms.helper;
...
public class ShowHelper {
	...
	private static Specification<ShowEntity> getShowByMovieNameSpec(String movieName) {
		return (root, query, criteriaBuilder) -> {
			Join<ShowEntity, MovieEntity> movieJoin = (Join<ShowEntity, MovieEntity>) root.join("movie");
	...
	private static Specification<ShowEntity> getShowByCitySpec(String city) {
		return (root, query, criteriaBuilder) -> {
			Join<ShowEntity, TheaterEntity> theaterJoin = (Join<ShowEntity, TheaterEntity>) root.join("theater");
			return criteriaBuilder.equal((Expression<?>) theaterJoin.get("city"), city);
		};
	}
